...
| DEC-12 | **Nova (21/09/2026), nasce de R-12/L-11.** Estratégia de alocação de lote no consumo de matéria-prima pela OS/OP: qual lote baixar primeiro quando existe mais de um em estoque? Trava a tarefa J3 (genealogia de material, S5). | Robert + Pablo | 20/11 | FIFO por `data_posicao` (lote mais antigo primeiro) | J3 |
...
| R-12 | ~~Genealogia de material: é exigência de cliente ou norma?~~ ✅ **DECIDIDO em 21/09/2026**: sim, é necessário (Nathan confirma a exigência, sem detalhar se é cliente específico ou norma). ⚠️ **Implicação de escopo real**: hoje **não existe** nenhum fluxo em que OS/OP debite saldo por lote de matéria-prima (ver L-11 em [[Lacunas-de-Logica-e-Clareza]]) — sem isso, a genealogia é impossível de construir. Isso é trabalho de engenharia novo, não coberto em nenhuma tarefa do [[Cronograma-2-Meses]] hoje (nem em S1-FC, nem na S5) — precisa ser dimensionado e encaixado, mesmo padrão usado pra rastreabilidade completa (S5). | Nathan + Qualidade |

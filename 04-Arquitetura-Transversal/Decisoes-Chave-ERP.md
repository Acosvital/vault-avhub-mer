...
- [x] **Vínculo Fábrica ↔ Unidade/Filial (`codigo_empresa`): por pedido, não fixo.** Decidido em 21/09/2026 por Nathan + Robert (resolve DEC-1) — existe hoje uma Fábrica real atendendo matriz e filiais, então "1 fábrica = 1 filial fixa" não reflete a operação. A filial de cada Ordem de Produção vem sempre do `codigo_empresa` do Pedido, nunca de um campo fixo na Fábrica. Gera requisito novo de RBAC (`PerfilSetor` precisa de dimensão de filial, não só `perfil × setor`) para as tarefas C2/C3. Ver [[MES-Arquitetura-Decisoes]].
...
- [x] **"Casamento" av-hub ↔ MES (termo do usuário) — resolvido.** Decidido em 21/09/2026 por Nathan + Robert + Gustavo (resolve DEC-2): **polling REST bidirecional, autenticado por `x-api-key`**, cobrindo 3 fluxos — requisição (MES→av-hub), referência da OC (av-hub→MES), status por item (MES→av-hub). Mesmo padrão já provado no pipeline ELT do Omie (sem webhook, sem tempo real) e mesma autenticação que a API do av-hub já usa. Detalhe completo em [[MES-Arquitetura-Decisoes]] item 8. **Histórico do achado de 17/09** (mantido por rastreabilidade): o PRD do Estoque original chegou a afirmar que a projeção de fornecedor/material do av-hub pro Estoque usaria "o mesmo mecanismo de evento que o av-hub já usa pra ler status de produção do MES" — contradição, porque esse mecanismo não existia ainda; agora existe (é este item), e é polling, não evento.

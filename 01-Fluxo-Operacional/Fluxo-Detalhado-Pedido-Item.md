...
> **Origem:** transcrição de 3 áudios do gerente (16/09) + resumo estruturado do usuário, cruzados e alinhados com o restante do vault nesta mesma rodada.
>
> **Relação com [[Fluxo-Operacional-Visao-Geral]]:** são **dois modelos complementares, não um substituindo o outro** (decisão explícita do usuário). O macro trata Estoque/Revenda/Fabricação como três categorias de destinação; este arquivo descreve o que acontece de fato, item a item, dentro dessas categorias — inclusive o achado de que "ter em estoque" na prática é uma **checagem** feita pelo PCP dentro de Revenda/Fabricação, não uma quarta rota isolada.
...
> ✅ **Resolvido (16/09):** os áudios não mencionavam reserva de saldo no momento em que o PCP marca um item como "tem em estoque" — risco de corrida entre pedidos concorrentes, já catalogado em [[Estoque-Riscos]] ("mesmo lote comprometido por PCP e por uma venda ao mesmo tempo"). Confirmado: a checagem do PCP passa a criar a reserva (`reserva_estoque`) no mesmo passo que marca "tem em estoque", não só verificar saldo de leitura. Falta desenhar o mecanismo de expiração/liberação da reserva.
...
## Divisão Compras av-hub × MES (decidido, 16/09)

...
Isso é o que o vault 01 se refere como `tipo_parceiro` "já sincronizado do Omie pelo pipeline ELT".

## Fornecedor

Não existe tabela separada — fornecedor reaproveita `core.parceiros` (mesma extração acima), decisão já confirmada no vault 01 ("fornecedor não tem cadastro próprio no Estoque").

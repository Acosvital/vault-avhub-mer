...
Vault de apoio, derivado de uma leitura direta do código-fonte do `omie-elt-pipeline` (não apenas da nota-resumo que já existe no vault 01). Objetivo: registrar **campo a campo** tudo que o pipeline extrai do Omie e grava no Postgres, e cruzar isso com o que o vault de modelagem técnica (`01 - vault-modelagem-desenvolvimento`) diz precisar — para achar lacunas reais antes de desenhar o módulo de Estoque/MES em cima de dados que talvez não existam.

Este vault não substitui a nota `Omie-ELT-Pipeline.md` do vault 01 (que cobre arquitetura, incidentes e filosofia do pipeline) — ele complementa com o nível de detalhe de coluna/campo que faltava.
...
- **2. Análise de Lacunas** — o que o vault 01 diz precisar e não está sendo extraído, contradições encontradas entre a documentação e o código real, colunas que nenhum dos dois lados escreve, e perguntas em aberto: [[Campos-Faltantes-para-Estoque-MES]], [[Contradicao-CFOP]], [[Colunas-Protegidas]], [[Perguntas-em-Aberto]].
...
- 🔴 **Contradição encontrada**: o vault 01 afirma que CFOP "fica 100% com o Omie" e não é capturado por nenhum sistema — mas o pipeline **captura e valida `cfop` por item** em `produto_vendas` (contra uma whitelist `core_vendas_faturamento.cfop`). Só ICMS-ST continua de fato não capturado. Ver [[Contradicao-CFOP]].
...
- 🟢 **Novo dado não documentado no vault 01**: `pedidos_vendas.codigo_projeto` e `.numero_contrato` são extraídos do Omie e já existem na tabela — podem ser úteis para o MES/Estoque sem esforço extra de integração.

## Ver também (em outro vault — abrir manualmente, wikilinks não atravessam vaults)
- `01 - vault-modelagem-desenvolvimento/03-Sistemas-Existentes/Omie-ELT-Pipeline.md`
- `01 - vault-modelagem-desenvolvimento/02-PRD-Estoque/Estoque-Modelo-Dados.md`
- `01 - vault-modelagem-desenvolvimento/04-Arquitetura-Transversal/Perguntas-Pendentes-MES-Estoque.md`

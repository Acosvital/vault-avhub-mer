...
> **Atualizado em 24/09/2026 com o encaixe do MES** ([[Encaixe-Estoque-Revenda-no-PCP]] seções 3.6 e 3.7): reserva passa a apontar para lote + `ItemParcial`, sem expiração; `movimento_estoque` ganha tipo, destino opcional e referência à origem; `deposito` ganha `codigo_empresa`; material liga ao item do pedido por `(codigo_empresa, id_omie)`.

- **material** — projeção de `core.produtos` (av-hub), mesmo padrão já decidido pra fornecedor (`core.parceiros`). Campos extras do lado do Estoque (tipo MATÉRIA_PRIMA/REVENDA, categoria, peso teórico unitário, tolerância de peso, estoque mínimo/máximo, ponto de pedido) podem vir do próprio Omie quando o dado já existir lá, ou virar colunas novas só do lado do Estoque quando o Omie não tiver o campo. **Ligação com o item do pedido (24/09/2026):** `(Pedidos.idUnidade, ItensPedido.idOmie)` = `(Material.codigoEmpresa, Material.codigoProdutoOmie)`, ambos UUID para a empresa (confirmar no banco). O tipo MATÉRIA_PRIMA/REVENDA do material **não decide a rota** do item — quem decide é a fábrica escolhida pelo PCP na rodada.
...
- **movimento_estoque** — **ganha tipo** (`ENTRADA` \| `SAIDA` \| `TRANSFERENCIA` \| `AJUSTE`), **destino opcional** e **referência à origem** (reserva, recebimento, OP) — decidido em 24/09/2026; o mock de `develop` só suporta `TRANSFERENCIA`/`AJUSTE`. `ENTRADA` registra o item comprado que volta da Qualidade para o Estoque; `SAIDA` consome a reserva na entrega. A J2 acrescenta `CONSUMO` (matéria-prima pela OS/OP).
- **lote** precisa de **depósito/localização atual** (ou uma tabela de saldo lote × localização) para o saldo por filial funcionar.
- **deposito** (`warehouse`), **localizacao_estoque**. Depósito é **central compartilhado em relação à fábrica** — não vinculado a uma fábrica específica, múltiplos warehouses (Warehouse 01, Warehouse 02...), cada um com saldo próprio; todas as fábricas e a Revenda puxam do mesmo conjunto. **Atualização (21/09/2026, L-09)**: em relação à **filial**, não é incondicional — `deposito.codigo_empresa` existe e é preenchido **quando aquela filial tiver seu próprio setor de compras**; não é regra universal desde o dia 1. Ver [[Perguntas-Pendentes-MES-Estoque]] (pergunta 4) e [[Lacunas-de-Logica-e-Clareza]] (L-09).
- **etiqueta** (código de barras/QR/RFID).
- **reserva_estoque** (`Reserva`) — impede dupla alocação entre PCP e Comercial. **Modelo decidido em 24/09/2026:** lote + `ItemParcial` (o split atendido) + quantidade + status (`ATIVA` \| `CONSUMIDA` \| `LIBERADA`). **Sem expiração** — liberação explícita quando o pedido/OP é cancelado. Substitui o `pedidoNumero` em texto e o status `EXPIRADA` do mock. Nasce sempre no **setor Estoque**, sobre lote liberado: na etapa 1 (saldo existente) ou na volta do item comprado depois da Qualidade. **Saldo disponível** = saldo do lote liberado − reservas `ATIVAS`.
...
## Ver também
- [[Encaixe-Estoque-Revenda-no-PCP]] — reserva por `ItemParcial`, tipos de movimento e setor Estoque no roteiro.

...
Corrigido nesta mesma rodada com links: Home do vault 01, [[Decisoes-Chave-ERP]], [[Cronograma-2-Meses]], [[Fluxo-Detalhado-Pedido-Item]], [[Diagramas-UML]] (seções 8 e 16), [[App-PCP-Backend-Producao]], [[Estoque-Modelo-Dados]], [[Glossario]], o vault 02 (linguagem de negócio), o vault 03 (`Campos-Faltantes-para-Estoque-MES`) e o índice do vault 04.
...
1. **Chave de correlação concreta.** O pipeline usa `(codigo_empresa, codigo_pedido_omie)` para o pedido e `(codigo_empresa, codigo_item_omie)` para o item (vault 03, `01-Dados-Extraidos/Pedidos-de-Venda.md`); o `sequencial` (0 = guarda-chuva, 1/2/... = parciais) cria uma linha de pedido por parcial. O MES hoje só guarda `pedido_venda` como texto e `ItemPedido.codigo`. Proposta: toda entidade rastreada carrega `codigo_empresa + numero_pedido + codigo_item_omie`. **(confirmar com Gustavo se o `codigo_item_omie` se mantém estável quando o pedido é parcializado.)**
...
### 3.2 No av-hub (`api-acos-vital`, schemas do cluster; colunas padrão `id uuidv7`, `created_at/by`, `updated_at/by`, `deleted_at/by`, como no contrato SQL 004 do vault 04)
...
### 4.2 av-hub
- **`pedidos_vendas`, `produto_vendas`, `pedidos_vendas_status_historico`: não alterar.** São do pipeline ou refletem o Omie com granularidade grossa (vault 03, `02-Analise-de-Lacunas/Campos-Faltantes-para-Estoque-MES.md`, item 5). O histórico fino nasce nas tabelas novas.
...
| `GET /produtos` e `/parceiros` com `?alterado_desde=` | Já previsto no contrato de API 001 do vault 04. |
...
1. **Robert:** quais colunas reais tem `HistoricoItemParcial`, `Operador` e `Divergencia`? O operador registrado numa ação é quem está logado ou alguém escolhido na tela?
2. **Nathan / setores:** SLA em horas corridas ou úteis? Se úteis, há calendário e turnos por setor?
3. **Gustavo:** `codigo_item_omie` é estável quando o pedido é parcializado? Os operadores de chão de fábrica existem em `core.funcionarios`?
4. **Nathan:** a previsão de chegada da OC volta ao MES junto da referência mínima (C7)? Ela é prazo, não dado comercial.
5. **Nathan:** o log guarda o nome do ator no evento (snapshot) e como isso convive com anonimização LGPD?
6. **Todos:** a projeção `item_acompanhado` nasce só para Recebimento, Qualidade e Compras (mínimo do ciclo 1) ou para todas as rotas desde o início?

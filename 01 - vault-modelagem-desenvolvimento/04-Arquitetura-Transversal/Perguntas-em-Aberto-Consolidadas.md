...
> Reúne, num só lugar, as perguntas em aberto espalhadas pelos 4 vaults, organizadas por **quem responde**. Levantada em 21/09/2026, um dia antes do início da execução. **A fonte de verdade continua sendo cada nota de origem** (coluna "Origem"); quando uma pergunta for respondida, atualizar lá e riscar aqui.
>
> Wikilinks não atravessam vaults: onde a origem está em outro vault, o caminho vem em texto.
...
Origem: `04 - vault-contratos/` (seção "Perguntas em aberto" de cada contrato). Nenhum contrato foi aplicado.
...
| I-02 | Contrato SQL 006 (`vTotal`, perguntas 1 e 2) e `03 - vault-integracao-omie/02-Analise-de-Lacunas/Perguntas-em-Aberto.md` (item 2) | Em [[Perguntas-Pendentes-MES-Estoque]], confirmado em 17/09/2026: `StatusDevolucaoVenda` **não está disponível**; o valor da devolução parcial terá de ser capturado nativamente. O contrato 006 e o item 2 do vault 03 partem de uma premissa invalidada, mas o índice do vault 04 ainda lista o 006 como "proposta". |
...
| I-04 | `Estoque-Perguntas-Abertas` e vault 02 (`Perguntas-em-Aberto`) | Repetem as mesmas 5 perguntas de negócio (tolerância, retenção, aprovação, carga inicial, balança), que são DEC-3 a DEC-6 e DEC-9. Uma só lista deveria valer. |

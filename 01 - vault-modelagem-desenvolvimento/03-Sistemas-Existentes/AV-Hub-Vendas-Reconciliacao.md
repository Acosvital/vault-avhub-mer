...
**Confirmado com o usuário (17/09/2026): não tem como vir do Omie.** A hipótese de buscar esse valor via API (endpoint `StatusDevolucaoVenda`, ver [[04 - vault-contratos/01-Contratos-SQL-DBA/006-Pedidos-Vendas-Valor-Devolucao|Contrato 011]]) foi testada e invalidada — o endpoint não está disponível. G2P vai continuar sem valor de devolução parcial confiável até que o sistema da Aços Vital capture esse valor nativamente (fora do Omie) e o alimente de volta pra essa cascata — não é mais uma questão de aguardar sincronização.

...
**Achado importante do "pente fino" no backend `api-acos-vital`**: existe um schema Postgres inteiro dedicado a comissionamento, com bem mais estrutura do que o protótipo frontend ([[AV-Hub-Simulador-Comissao]]) sugeria. Isso muda a pergunta em aberto de "[[Decisoes-Chave-ERP|Simulador de Comissão é a mesma coisa que o projeto Python de automação, ou são paralelos?]]" — agora sabemos que existe pelo menos um **terceiro** pedaço da resposta: um schema de produção já em uso.

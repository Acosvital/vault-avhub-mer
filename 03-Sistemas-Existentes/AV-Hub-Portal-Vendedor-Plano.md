...
Fonte principal: `GET /nf_classified` (não `notas_fiscais` cru; o nome real da tabela não é `nota_fiscal_saida`) — já vem com `grupo_deducao` pronto por nota. Campos mostrados: `numero_nf`, `data_emissao`/`hora_emissao`, `valor_nf`/`valor_mercadorias`/`valor_ipi`, vínculo com o pedido de origem. Removido a pedido do Nathan: `averbado`.
